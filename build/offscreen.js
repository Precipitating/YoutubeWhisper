...
function M(c) {
  ...
  let r = 0, o = 0, n = 0;
  if (t.length === 3)
    [r, o] = t, n = parseFloat(e.split(":").pop() || "0");
  else if (t.length === 2)
    [o] = t, n = parseFloat(e.split(":").pop() || "0");
  else
    throw new Error(`Bad time format: "${c}"`);
  return Math.floor(((r * 60 + o) * 60 + n) * 1e3);
}
function B(c) {
  ...
  const r = t[1], o = t[2], n = t[3] || "", s = M(r), i = M(o);
  if (i < s)
...
function R(c, e) {
  let t = null, r = false, o = null, n = null;
  return { timeoutError: () => new Promise((a, g) => {
    n = a, o = g, t = setTimeout(() => {
      !r && o && (r = true, o(new Error(e)));
    }, c);
  }), clear: () => {
    t && (clearTimeout(t), t = null), n && (n(), n = null), r = true, o = null;
  } };
}
function x(c, e = 16e3 * 100) {
  const t = [];
...
class L {
  ...
  async *streamimg(e, t = {}) {
    const { timeoutMs: r = 3e4 } = t, o = x(e);
    let n = 0;
    for await (const s of o) {
      const i = [];
      let a = null, g = false, d, f = 0;
      const { timeoutError: m, clear: u } = R(r, "Transcribe timeout"), h = () => this.whisperService.transcribe(
        s,
        (l) => {
          f = l.timeEnd, l.timeStart += n, l.timeEnd += n, this.logger.debug("Transcription segment in session:", l), a ? (a(l), a = null) : i.push(l), u();
        },
        t
      ).then(() => {
        this.logger.debug("Transcription done in session then"), g = true, n += f, u(), a == null || a(void 0);
      }).catch((l) => {
...
class A {
  constructor(e) {
    this.wasmModule = null, this.instance = null, this.modelFileName = "whisper.bin", this.isTranscribing = false, this.bus = new D(), this.modelData = null, this.logger = new b((e == null ? void 0 : e.logLevel) ?? b.levels.ERROR, "WhisperWasmService"), e != null && e.init && this.loadWasmScript();
  }
  async checkWasmSupport() {
    return await z();
  }
  async loadWasmScript() {
    this.wasmModule = await (await import('./libmain-D9-QM3iM-BP_0jJOG.js')).default({
      print: (e, ...t) => {
  ...
  storeFS(e, t) {
    ...
    this.wasmModule.FS_createDataFile("/", e, t, true, true, true);
  }
  async transcribe(e, t, r = {}) {
    ...
    return this.wasmModule.full_default(this.instance, e, n, s, i), await new Promise((d, f) => {
      const m = this.bus.on("transcribe", (l) => {
        ...
        () => {
          this.isTranscribing = false, m(), h(), this.logger.error("Transcribe timeout"), f(new Error("Transcribe timeout")), this.bus.emit("transcribeError", "Transcribe timeout");
        },
        o * 2 * 1e3
      ), h = this.bus.on("transcribeError", (l) => {
        this.isTranscribing = false, m(), h(), clearTimeout(u), this.logger.debug("Transcribe error", l.detail), d({ segments: a, transcribeDurationMs: Date.now() - g });
      });
    });
  }
  createSession() {
    return new L(this, { logLevel: this.logger.getLevel() });
  }
}
const S = {
  "tiny.en": {
...
class F {
  constructor(e = { logLevel: b.levels.ERROR }) {
    this.cacheEnabled = true, this.models = W(), this.logger = new b(e.logLevel, "ModelManager");
  }
  /**
   * Loads model by name
   */
  async loadModel(e, t = true, r) {
    var h;
    const o = E(e);
    if (!o)
    ...
    const n = await fetch(o.url);
    if (!n.ok)
      throw new Error(`Failed to load model: ${n.statusText}`);
    const s = n.headers.get("content-length"), i = s ? parseInt(s, 10) : 0;
    let a = 0;
    const g = (h = n.body) == null ? void 0 : h.getReader();
    if (!g)
  ...
  async loadModelByUrl(e, t) {
    ...
    try {
      ...
      const n = o.headers.get("content-length"), s = n ? parseInt(n, 10) : 0;
      let i = 0;
      ...
      try {
        ...
        for (; !u; ) {
          ...
          if (u = h.done, !u && h.value && (g.push(h.value), i += h.value.length, t && s > 0)) {
  ...
  async saveModelToCacheByUrl(e, t) {
    try {
      const n = (await this.openIndexedDB()).transaction(["modelsByUrl"], "readwrite").objectStore("modelsByUrl");
  ...
  async saveModelToCache(e, t) {
    try {
      const n = (await this.openIndexedDB()).transaction(["models"], "readwrite").objectStore("models");
      await new Promise((s, i) => {
        const a = n.put({
          name: e,
          data: t,
  ...
  async openIndexedDB() {
    return new Promise((e, t) => {
      ...
      r.onerror = () => t(r.error), r.onsuccess = () => e(r.result), r.onupgradeneeded = (o) => {
        ...
        if (!n.objectStoreNames.contains("models")) {
          const s = n.createObjectStore("models", { keyPath: "name" });
          s.createIndex("timestamp", "timestamp", { unique: false }), s.createIndex("size", "size", { unique: false });
        }
        if (!n.objectStoreNames.contains("modelsByUrl")) {
          const s = n.createObjectStore("modelsByUrl", { keyPath: "url" });
          s.createIndex("timestamp", "timestamp", { unique: false }), s.createIndex("size", "size", { unique: false });
        }
      };
    });
  ...
  async getCacheInfo() {
    try {
      ...
      return new Promise((o, n) => {
        const s = r.getAll();
        s.onsuccess = () => {
          const i = s.result, a = i.reduce((g, d) => g + (d.size || 0), 0);
